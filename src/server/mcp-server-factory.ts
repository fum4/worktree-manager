  ...
  server.prompt(
    ...
    async ({ issueId }) => ({
      messages: [{
        ...
        content: {
          type: 'text' as const,
          text: `Use the dawg MCP server tools to create a worktree for issue "${issueId}". Follow this workflow:
1. Determine the issue type and call the appropriate dawg tool: create_from_jira or create_from_linear
2. The response will include full task context and the worktree path
3. Poll the dawg list_worktrees tool until the worktree status is 'stopped' (creation complete)
...
Skill reports: For skills with detailed output (code review, changes summary, test instructions, explanations), write the full report to {worktreePath}/.dawg-{skillName}.md and pass the path via filePath in report_hook_status.
